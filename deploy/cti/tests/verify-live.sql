...
DO $$
BEGIN
    IF COALESCE((SELECT max(version) FROM cti.schema_versions), 0) < 15 THEN
        RAISE EXCEPTION 'CTI schema version 15 is not installed.';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        WHERE name = 'Microsoft Security Blog'
          ...
          AND allowed_hosts = ARRAY['www.microsoft.com']
